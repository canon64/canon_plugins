...
namespace MainGameVoiceFaceEventBridge
{
    ...
    internal sealed class ExternalVoicePlayer : IDisposable
    {
            ...
            float pitch)
        {
            ...
            string normalizedSessionId = sessionId ?? string.Empty;
            if (!string.IsNullOrEmpty(_sequenceSessionId)
                && !string.Equals(_sequenceSessionId, normalizedSessionId, StringComparison.Ordinal))
            {
                _logWarn?.Invoke("[audio-seq] append rejected session=" + normalizedSessionId + " active=" + _sequenceSessionId);
                return false;
            }

            if (string.IsNullOrEmpty(_sequenceSessionId))
            {
                _sequenceSessionId = normalizedSessionId;
            }
            ...
            lock (_queueLock)
            {
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    if (item.SequencePosition <= 0)
                    {
                        item.SequencePosition = item.Index > 0 ? item.Index : accepted + 1;
                    }

                    if (item.Index <= 0)
                    {
                        item.Index = item.SequencePosition;
                    }

                    if (item.Total <= 0)
                    {
                        item.Total = Math.Max(item.Index, item.SequencePosition);
                    }
            ...
            lock (_queueLock)
            {
                shouldStart = _pendingQueueItem == null;
            }
            ...
            _logInfo?.Invoke("[audio-seq] appended session=" + _sequenceSessionId + " count=" + accepted);
            return true;
        }
