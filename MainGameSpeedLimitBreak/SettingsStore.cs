...
namespace MainGameSpeedLimitBreak
{
    internal static class SettingsStore
    {
        ...
        internal static PluginSettings LoadOrCreate(string pluginDir, Action<string> info, Action<string> warn, Action<string> error)
        {
            string path = Path.Combine(pluginDir, "SpeedLimitBreakSettings.json");
            try
            {
                if (!File.Exists(path))
                {
