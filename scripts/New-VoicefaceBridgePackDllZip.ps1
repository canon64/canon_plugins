param(
    ...
    [string]$SourceRootReadmePath = 'F:\kks\work\plugins\canon_plugins\README_voiceface_bridge_pack_source.md',
    ...
    [string]$SourceDeployTestRootBase = 'F:\kks\work\_tmp\voiceface_bridge_pack_dll_zip\_source_deploy_test',
    [switch]$GitCommitOnProduction,
    [switch]$GitPushOnProduction,
    [string]$GitCommitMessage = 'ボイスフェイスセットソース更新',
    [string]$GitBranch = '',
    [string]$ExpectedProductionRemote = 'github.com/canon64/kks-voiceface-bridge-pack',
...
if ([string]::IsNullOrWhiteSpace($ZipName)) {
    $ZipName = ('voiceface_bridge_pack_dll_{0}.zip' -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
}

New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null

$runStamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$stagingRoot = Join-Path $OutputDir ('_staging_voiceface_bridge_pack_{0}' -f $runStamp)
