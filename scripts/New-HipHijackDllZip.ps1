param(
    ...
    [string]$SourceRootReadmePath = 'F:\kks\work\plugins\canon_plugins\README_hiphijack_source.md',

    [string]$OutputDir = 'F:\kks\work\_tmp\hiphijack_dll_zip',
    [string]$ZipName = '',
    [string]$SourceDeployRoot = 'F:\kks\work\_tmp\MainGirlHipHijack_release_work',
    [string]$SourceDeployTestRootBase = 'F:\kks\work\_tmp\hiphijack_dll_zip\_source_deploy_test',
    [switch]$GitCommitOnProduction,
    [switch]$GitPushOnProduction,
    [string]$GitCommitMessage = 'ヒップハイジャックソース更新',
    [string]$GitBranch = '',
    [string]$ExpectedProductionRemote = 'github.com/canon64/MainGirlHipHijack',
...
if ([string]::IsNullOrWhiteSpace($ZipName)) {
    $ZipName = ('hiphijack_dll_{0}.zip' -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
}

New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null

$runStamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$stagingRoot = Join-Path $OutputDir ('_staging_hiphijack_{0}' -f $runStamp)
