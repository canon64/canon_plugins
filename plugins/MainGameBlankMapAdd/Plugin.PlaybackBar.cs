...
namespace MainGameBlankMapAdd
{
    public sealed partial class Plugin
    {
        ...
        private void OnGUI()
        {
            ...
            bool dropdownOpen = _settings.FolderPlayEnabled && (_folderDropdownOpen || _videoDropdownOpen);
            ...
            if (_playbackReverbDragging && !holdingMouse)
            {
                _playbackReverbDragging = false;
                CommitPlaybackBarReverbMaxDistance();
            }
            if (_playbackBarHiddenByUser &&
                !_playbackSeekDragging &&
                !_playbackVolumeDragging &&
                !_playbackGainDragging &&
                !_playbackReverbDragging)
                return;

            bool shouldShow = mouseInTrigger || mouseOverBar || dropdownOpen ||
                _playbackSeekDragging || _playbackVolumeDragging || _playbackGainDragging || _playbackReverbDragging;
            ...
            bool nextHelpPopup = HelpToggle(
                new Rect(panelToggleX, helpToggleY, helpToggleW, helpToggleH),
                currentHelpPopup,
                "説明",
                "説明ポップアップ表示");
            ...
            if ((_folderDropdownOpen || _videoDropdownOpen) &&
                Event.current != null &&
                Event.current.type == EventType.MouseDown)
            {
                ...
                if (!insideFolderUi && !insideVideoUi)
                {
                    _folderDropdownOpen = false;
                    _videoDropdownOpen = false;
                }
            ...
            float volumeSliderWidth = Mathf.Clamp(sliderRowWidth * 0.10f, 56f, 88f); // Keep volume slider smaller.
            float reverbTextWidth = 58f;
            float reverbSliderWidth = Mathf.Clamp(sliderRowWidth * 0.10f, 56f, 88f);

            float reverbSliderX = sliderRowEndX - reverbRightInsetX - reverbSliderWidth;
            float reverbTextX = reverbSliderX - sliderGap - reverbTextWidth;
            float volumeSliderX = reverbTextX - sliderGap - volumeSliderWidth;
            ...
            var reverbTextRect = new Rect(reverbTextX, sliderLabelY, reverbTextWidth, rowH);
            var reverbSliderRect = new Rect(reverbSliderX, sliderY, reverbSliderWidth, 12f);
            float videoReverbToggleWidth = Mathf.Clamp(reverbRightInsetX - 4f, 44f, 92f);
            var videoReverbToggleRect = new Rect(
                reverbSliderRect.xMax + 4f,
                sliderLabelY,
            ...
            if (Event.current != null &&
                Event.current.type == EventType.MouseDown &&
                reverbSliderRect.Contains(mouseGui))
            {
                _playbackReverbDragging = true;
            }
        ...
        private void ApplyPlaybackBarReverbMaxDistance(float maxDistance, bool persist)
        {
            if (_settings == null)
                return;
        ...
        private void CommitPlaybackBarReverbMaxDistance()
        {
            if (_settings == null)
                return;
            ApplyPlaybackBarReverbMaxDistance(_settings.VoiceReverbMaxDistance, persist: true);
        }
