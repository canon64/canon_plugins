    ...
    public sealed partial class Plugin
    {
        ...
        private bool TryApplyBeatSyncSnapshot(BeatSyncSnapshot snapshot, string reason)
        {
            return TryApplyBeatSyncSnapshot(
                ...
                applyGlobalAutoMotionSwitch: true,
                applyGlobalAutoThreshold: true,
                applyGlobalVerboseLog: true);
        }
        ...
        private bool TryApplyBeatSyncSnapshot(
            ...
            bool applyGlobalAutoMotionSwitch,
            bool applyGlobalAutoThreshold,
            bool applyGlobalVerboseLog)
        {
            if (snapshot == null)
                return false;
        ...
        // ── MainGameSubCameraDisplayProbe UI表示 ─────────────────────

        private bool TryGetSubCameraDisplayProbeUiVisible(out bool visible)
        {
            visible = false;
            try
            {
                if (!TryGetSubCameraDisplayProbePluginInstance(out object instance, out Type instType))
                    return false;
                ...
                const BindingFlags flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
                FieldInfo cfgShowField = instType.GetField("_cfgShowWindow", flags);
                if (cfgShowField != null)
                {
                    visible = GetConfigEntryBool(instType, instance, "_cfgShowWindow", false);
                    return true;
                }
            ...
            {
                LogWarn($"subcam ui snapshot failed: {ex.Message}");
                return false;
            }
        }

        private bool TryApplySubCameraDisplayProbeUiVisible(bool visible, string reason)
        {
            try
            {
                if (!TryGetSubCameraDisplayProbePluginInstance(out object instance, out Type instType))
                ...
                if (setter != null && setter.ReturnType == typeof(bool))
                {
                    ...
                    if (raw is bool b && b)
                    {
                        LogInfo($"subcam ui applied reason={reason} visible={visible} mode=api");
                        return true;
                    }
                }
                ...
                if (cfgShowField != null)
                {
                    SetConfigEntryBool(instType, instance, "_cfgShowWindow", visible);
                    LogInfo($"subcam ui applied reason={reason} visible={visible} mode=field-show-window");
                    return true;
                }
                ...
                {
                    SetConfigEntryBool(instType, instance, "_cfgUiVisible", visible);
                    LogInfo($"subcam ui applied reason={reason} visible={visible} mode=field-ui-visible");
                    return true;
                }
                ...
                uiVisibleField.SetValue(settings, visible);
                LogInfo($"subcam ui applied reason={reason} visible={visible} mode=settings-field");
                return true;
            }
            catch (Exception ex)
            {
                LogWarn($"subcam ui apply failed reason={reason} error={ex.Message}");
                return false;
            }
        }

        private bool TryGetSubCameraDisplayProbePluginInstance(out object instance, out Type instanceType)
        {
            instance = null;
            instanceType = null;
            try
            {
                if (Chainloader.PluginInfos != null &&
                    Chainloader.PluginInfos.TryGetValue("com.kks.maingame.subcameradisplayprobe", out var pluginInfo))
                {
                    instance = pluginInfo?.Instance;
                    if (instance != null)
                    {
        ...
        // ── MainGameObjectComposer UI表示 ─────────────────────────────

        private bool TryGetObjectComposerPluginInstance(out object instance, out Type instanceType)
        {
            instance = null;
            instanceType = null;
            try
            {
                if (Chainloader.PluginInfos != null &&
                    Chainloader.PluginInfos.TryGetValue("com.kks.main.gameobjectcomposer", out var pluginInfo))
                {
                    instance = pluginInfo?.Instance;
                    if (instance != null)
                    {
        ...
        private bool TryGetObjectComposerUiVisible(out bool visible)
        {
            visible = false;
            try
            {
                if (!TryGetObjectComposerPluginInstance(out object instance, out Type instType))
                    return false;
            ...
            {
                LogWarn($"object-composer ui snapshot failed: {ex.Message}");
                return false;
            }
        }

        private bool TryApplyObjectComposerUiVisible(bool visible, string reason)
        {
            try
            {
                if (!TryGetObjectComposerPluginInstance(out object instance, out Type instType))
                ...
                if (setter != null && setter.ReturnType == typeof(bool))
                {
                    ...
                    if (raw is bool b && b)
                    {
                        LogInfo($"object-composer ui applied reason={reason} visible={visible} mode=api");
                        return true;
                    }
                }
                return false;
            ...
            {
                LogWarn($"object-composer ui apply failed reason={reason} error={ex.Message}");
                return false;
            }
        }
