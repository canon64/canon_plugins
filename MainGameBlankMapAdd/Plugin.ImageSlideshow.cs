...
namespace MainGameBlankMapAdd
{
    public sealed partial class Plugin
    {
        ...
        private const string ImageSlideshowQuadMeshName = "ImageSlideshowQuadMesh";
        ...
        private void ForceShowLatestImageSlideshow()
        {
            ...
            {
                // TryShowLatestImageSlideshow() may start a transition.
                // In that case, do not reset the current/base image display start time.
                // Resetting _imageSlideshowDisplayStartTime here makes the outgoing image jump
                // back to the beginning of its pan (for example Top->Bottom), which looks unnatural.
                if (_imageSlideshowTransitionActive)
                {
                    float transitionStart = _imageSlideshowTransitionDisplayStartTime >= 0f
                        ? _imageSlideshowTransitionDisplayStartTime
                        : Time.unscaledTime;
                    ScheduleNextImageSlideshowSlideTime(transitionStart);
                    LogImageSlideshowVerbose("[slideshow] force show latest transition path=" + (_imageSlideshowTransitionPath ?? string.Empty));
                }
                else
                {
                    _imageSlideshowDisplayStartTime = Time.unscaledTime;
            ...
            float surfaceOffset)
        {
            ...
            MakeImageSlideshowQuadOwnMesh(quad);
        ...
        private static void MakeImageSlideshowQuadOwnMesh(GameObject quad)
        {
            if (quad == null)
                return;
            ...
            Mesh ownedMesh = UnityEngine.Object.Instantiate(meshFilter.sharedMesh);
            ownedMesh.name = ImageSlideshowQuadMeshName;
            meshFilter.sharedMesh = ownedMesh;
        }

        private static void DestroyImageSlideshowQuad(GameObject quad)
        {
            if (quad == null)
                return;
            ...
            if (meshFilter != null)
            {
                ...
                if (mesh != null && string.Equals(mesh.name, ImageSlideshowQuadMeshName, StringComparison.Ordinal))
                {
        ...
        {
            for (int i = 0; i < _imageSlideshowQuads.Count; i++)
                DestroyImageSlideshowQuad(_imageSlideshowQuads[i]);
        ...
        {
            for (int i = 0; i < _imageSlideshowTransitionQuads.Count; i++)
                DestroyImageSlideshowQuad(_imageSlideshowTransitionQuads[i]);
