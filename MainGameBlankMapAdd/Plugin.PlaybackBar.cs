...
namespace MainGameBlankMapAdd
{
    public sealed partial class Plugin
    {
        ...
        private bool _scenarioTextSnapshotBarAvailable;
        private bool _scenarioTextAutoSendSnapshotBarEnabled;
        private bool _cameraSnapshotBarAvailable;
        private bool _cameraSnapshotBarVisible;
        private bool _subCamSnapshotBarAvailable;
        private bool _subCamSnapshotBarVisible;
        private bool _autoHVoiceSnapshotBarAvailable;
        private bool _autoHVoiceSnapshotBarVisible;
        private bool _objComposerSnapshotBarAvailable;
        private bool _objComposerSnapshotBarVisible;
        ...
        private void RefreshExternalUiSnapshotBarCacheIfNeeded()
        {
            ...
            _scenarioTextSnapshotBarAvailable = TryGetVfebScenarioTextAutoSendEnabled(out _scenarioTextAutoSendSnapshotBarEnabled);
            _cameraSnapshotBarAvailable = TryGetCameraControlUiVisible(out _cameraSnapshotBarVisible);
            _subCamSnapshotBarAvailable = TryGetSubCameraDisplayProbeUiVisible(out _subCamSnapshotBarVisible);
            _autoHVoiceSnapshotBarAvailable = TryGetAutoHVoiceUiVisible(out _autoHVoiceSnapshotBarVisible);
            _objComposerSnapshotBarAvailable = TryGetObjectComposerUiVisible(out _objComposerSnapshotBarVisible);
        }
        ...
        private void InvalidateExternalUiSnapshotBarCache()
        {
            ...
            _scenarioTextSnapshotBarAvailable = false;
            _scenarioTextAutoSendSnapshotBarEnabled = false;
            _cameraSnapshotBarAvailable = false;
            _cameraSnapshotBarVisible = false;
            _subCamSnapshotBarAvailable = false;
            _subCamSnapshotBarVisible = false;
            _autoHVoiceSnapshotBarAvailable = false;
            _autoHVoiceSnapshotBarVisible = false;
            _objComposerSnapshotBarAvailable = false;
            _objComposerSnapshotBarVisible = false;
        }
        ...
        private void OnGUI()
        {
            ...
            float slideshowSectionH = 138f;
            float overlaySectionH = 116f;
            float controlsGapY = 4f;
            float controlsTotalH = showRoomControls
                ? (layoutSectionH + controlsGapY + integrationSectionH + controlsGapY + slideshowSectionH + controlsGapY + overlaySectionH)
                : 0f;
            // 展開時: 詳細パネル最下段(SubCameraOverlay)が宣言高さを超えて標準パネルへ約20px食い込むため、
            // 詳細→標準のギャップを広げて重なりを解消する（barHeightも+28しているので標準行の絶対位置は据え置き＝詳細だけ上がる）。
            float y = showRoomControls ? (barRect.y + 4f + controlsTotalH + 36f) : (barRect.y + 8f);
            ...
            if (showRoomControls)
            {
                ...
                float overlayTopY = integrationTopY + integrationSectionH + controlsGapY;
                DrawImageSlideshowSection(beatSyncX, overlayTopY, beatSyncW, slideshowSectionH);
                overlayTopY += slideshowSectionH + controlsGapY;
                DrawSubCameraOverlaySection(beatSyncX, overlayTopY, beatSyncW, overlaySectionH);
            }
            ...
            float scenarioAutoToggleW = 82f;
            float scenarioManualButtonW = 78f;
            float cameraToggleW = 64f;
            float subCamToggleW = 64f;
            float autoHVoiceToggleW = 78f;
            float objComposerToggleW = 78f;
            float toggleGap = 4f;
            float panelToggleX = x - panelToggleW - pad;
            float toggleGroupW = helpToggleW + hipUiToggleW + clubUiToggleW + afterimageToggleW + dollModeToggleW + poseToggleW + scenarioAutoToggleW + scenarioManualButtonW + cameraToggleW + subCamToggleW + autoHVoiceToggleW + objComposerToggleW + toggleGap * 11f;
            ...
            float scenarioAutoToggleX = poseToggleX + poseToggleW + toggleGap;
            float scenarioManualButtonX = scenarioAutoToggleX + scenarioAutoToggleW + toggleGap;
            float cameraToggleX = scenarioManualButtonX + scenarioManualButtonW + toggleGap;
            float subCamToggleX = cameraToggleX + cameraToggleW + toggleGap;
            float autoHVoiceToggleX = subCamToggleX + subCamToggleW + toggleGap;
            float objComposerToggleX = autoHVoiceToggleX + autoHVoiceToggleW + toggleGap;
            ...
            bool scenarioTextAvailable = _scenarioTextSnapshotBarAvailable;
            bool scenarioAutoEnabledNow = _scenarioTextAutoSendSnapshotBarEnabled;
            GUI.enabled = prevGuiEnabled && scenarioTextAvailable;
            bool nextScenarioAutoEnabled = HelpToggle(
                new Rect(scenarioAutoToggleX, helpToggleY, scenarioAutoToggleW, helpToggleH),
                scenarioAutoEnabledNow,
                "状況Auto",
                scenarioTextAvailable
                    ? "VoiceFaceEventBridge 状況文の定期送信ON/OFF"
                    : "VoiceFaceEventBridge 状況文送信APIが未ロード");
            GUI.enabled = prevGuiEnabled;
            if (scenarioTextAvailable && nextScenarioAutoEnabled != scenarioAutoEnabledNow)
            {
                if (TryApplyVfebScenarioTextAutoSendEnabled(nextScenarioAutoEnabled, "bar-auto-toggle"))
                {
                    _scenarioTextAutoSendSnapshotBarEnabled = nextScenarioAutoEnabled;
                }
                else
                {
                    InvalidateExternalUiSnapshotBarCache();
            ...
            GUI.enabled = prevGuiEnabled && scenarioTextAvailable;
            bool scenarioManualClicked = HelpButton(
                new Rect(scenarioManualButtonX, helpToggleY, scenarioManualButtonW, helpToggleH),
                "状況送信",
                scenarioTextAvailable
                    ? "VoiceFaceEventBridge 状況文を今すぐ手動送信"
                    : "VoiceFaceEventBridge 状況文送信APIが未ロード");
            GUI.enabled = prevGuiEnabled;
            if (scenarioManualClicked)
            {
                if (!TrySendVfebScenarioTextManualNow("bar-manual-button"))
                {
                    InvalidateExternalUiSnapshotBarCache();
                }
            }

            bool cameraAvailable = _cameraSnapshotBarAvailable;
            bool cameraUiVisibleNow = _cameraSnapshotBarVisible;
            GUI.enabled = prevGuiEnabled && cameraAvailable;
            bool nextCameraUiVisible = HelpToggle(
                new Rect(cameraToggleX, helpToggleY, cameraToggleW, helpToggleH),
                cameraUiVisibleNow,
                "CameraUI",
                cameraAvailable
                    ? "MainGameCameraControl UI の表示/非表示"
                    : "MainGameCameraControl が未ロード");
            GUI.enabled = prevGuiEnabled;
            if (cameraAvailable && nextCameraUiVisible != cameraUiVisibleNow)
            {
                if (TryApplyCameraControlUiVisible(nextCameraUiVisible, "bar-ui-toggle"))
                {
                    _cameraSnapshotBarVisible = nextCameraUiVisible;
                }
                else
                {
                    InvalidateExternalUiSnapshotBarCache();
            ...
            bool subCamAvailable = _subCamSnapshotBarAvailable;
            bool subCamUiVisibleNow = _subCamSnapshotBarVisible;
            GUI.enabled = prevGuiEnabled && subCamAvailable;
            bool nextSubCamUiVisible = HelpToggle(
                new Rect(subCamToggleX, helpToggleY, subCamToggleW, helpToggleH),
                subCamUiVisibleNow,
                "SubCam",
                subCamAvailable
                    ? "MainGameSubCameraDisplayProbe UI の表示/非表示"
                    : "MainGameSubCameraDisplayProbe が未ロード");
            GUI.enabled = prevGuiEnabled;
            if (subCamAvailable && nextSubCamUiVisible != subCamUiVisibleNow)
            {
                if (TryApplySubCameraDisplayProbeUiVisible(nextSubCamUiVisible, "bar-ui-toggle"))
                {
                    _subCamSnapshotBarVisible = nextSubCamUiVisible;
                }
                else
                {
                    InvalidateExternalUiSnapshotBarCache();
            ...
            bool autoHVoiceAvailable = _autoHVoiceSnapshotBarAvailable;
            bool autoHVoiceUiVisibleNow = _autoHVoiceSnapshotBarVisible;
            GUI.enabled = prevGuiEnabled && autoHVoiceAvailable;
            bool nextAutoHVoiceUiVisible = HelpToggle(
                new Rect(autoHVoiceToggleX, helpToggleY, autoHVoiceToggleW, helpToggleH),
                autoHVoiceUiVisibleNow,
                "AutoVoice",
                autoHVoiceAvailable
                    ? "MainGameAutoHVoice UI の表示/非表示"
                    : "MainGameAutoHVoice が未ロード");
            GUI.enabled = prevGuiEnabled;
            if (autoHVoiceAvailable && nextAutoHVoiceUiVisible != autoHVoiceUiVisibleNow)
            {
                if (TryApplyAutoHVoiceUiVisible(nextAutoHVoiceUiVisible, "bar-ui-toggle"))
                {
                    _autoHVoiceSnapshotBarVisible = nextAutoHVoiceUiVisible;
                }
                else
                {
                    InvalidateExternalUiSnapshotBarCache();
            ...
            bool objComposerAvailable = _objComposerSnapshotBarAvailable;
            bool objComposerUiVisibleNow = _objComposerSnapshotBarVisible;
            GUI.enabled = prevGuiEnabled && objComposerAvailable;
            bool nextObjComposerUiVisible = HelpToggle(
                new Rect(objComposerToggleX, helpToggleY, objComposerToggleW, helpToggleH),
                objComposerUiVisibleNow,
                "Composer",
                objComposerAvailable
                    ? "MainGameObjectComposer UI の表示/非表示"
                    : "MainGameObjectComposer が未ロード");
            GUI.enabled = prevGuiEnabled;
            if (objComposerAvailable && nextObjComposerUiVisible != objComposerUiVisibleNow)
            {
                if (TryApplyObjectComposerUiVisible(nextObjComposerUiVisible, "bar-ui-toggle"))
                {
                    _objComposerSnapshotBarVisible = nextObjComposerUiVisible;
                }
                else
                {
                    InvalidateExternalUiSnapshotBarCache();
        ...
        private void ApplyGlobalReverbState(string source, bool persistConfig)
        {
            ...
            bool enabled = _settings.EnableVoiceReverb;
            ...
            ApplyPlaybackBarReverbMixToPlayingSources(enabled ? 1f : 0f);
            if (_videoRoomAudioSource != null)
                ConfigureVideoRoomAudioSource(_videoRoomAudioSource);
            ...
            if (persistConfig)
            {
                ...
                try
                {
                    if (_cfgEnableVoiceReverb != null)
                        _cfgEnableVoiceReverb.Value = enabled;
                }
                finally
                {
                    _syncingConfig = prevSync;
            ...
            LogInfo($"[reverb-state] source={source} enabled={enabled}");
        }
