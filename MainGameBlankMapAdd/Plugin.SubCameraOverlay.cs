...
namespace MainGameBlankMapAdd
{
    public sealed partial class Plugin
    {
        ...
        private bool TryGetBeatLoopHzForOverlay(out float hz)
        {
            ...
            float zoneSpeed01 = (dLow <= dMid && dLow <= dHigh) ? low
                : (dHigh <= dLow && dHigh <= dMid) ? high
                : mid;
        ...
        private float ResolveOverlayOpacityWithBeat()
        {
            ...
            if (_settings.OverlayBeatOpacityInverted)
                return Mathf.Lerp(max, min, pulse01);
            return Mathf.Lerp(min, max, pulse01);
        }
