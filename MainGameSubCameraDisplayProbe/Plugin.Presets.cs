...
namespace MainGameSubCameraDisplayProbe
{
    public sealed partial class Plugin
    {
        ...
        private void OverwriteActivePreset()
        {
            ...
            preset.BoneName = _activeBoneName;
            preset.SaveCameraPosition = _activeSaveCameraPosition;
            preset.LookAtOffsetLocal = ToArray(_activeLookAtOffsetLocal);
            preset.LookAtPosition = ToArray(bone.TransformPoint(_activeLookAtOffsetLocal));
            preset.CameraRotation = ToArray(NormalizeEuler(cameraTransform.rotation.eulerAngles));
            preset.Fov = _settings.CameraFieldOfView;

            if (_activeSaveCameraPosition)
            {
                preset.CameraPosition = ToArray(cameraTransform.position);
                preset.CameraOffsetLocal = ToArray(_activeCameraOffsetLocal);
            }
            else
            {
                preset.CameraPosition = null;
                preset.CameraOffsetLocal = null;
            }

            SaveSettings();
            SetStatus("上書き保存: " + (preset.Name ?? string.Empty));
        }
        ...
        {
            if (!_boneFollowActive || preset == null || !preset.UseBoneLink)
                return false;
