...
namespace MainGameClubLights
{
    ...
    public sealed class ClubLightsSettings
    {
        ...
        // フォルダ（全ライトの親）。位置・回転をワールドで保持。既定 identity（=従来互換、local==world）
        [OptionalField] public float FolderPosX = 0f;
        [OptionalField] public float FolderPosY = 0f;
        [OptionalField] public float FolderPosZ = 0f;
        [OptionalField] public float FolderRotX = 0f;
        [OptionalField] public float FolderRotY = 0f;
        [OptionalField] public float FolderRotZ = 0f;
        [OptionalField] public bool  ShowFolderHandle = true;  // 掴みハンドル（球＋ギズモ）の表示
        [OptionalField] public float FolderHandleSize = 0.25f;
