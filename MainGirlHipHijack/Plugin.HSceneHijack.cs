...
namespace MainGirlHipHijack
{
    public sealed partial class Plugin
    {
        ...
        {
            bool forceZeroByHipLock = _bodyCtrlLinkEnabled;
            bool cutBySetting = _settings != null && _settings.CutFemaleAnimSpeedEnabled;
            if ((!forceZeroByHipLock && !cutBySetting) || action == null)
                return false;
            ...
            // 腰リンク中はBPM計算を止めず、最終的な女性側アニメ速度だけ0に固定する。
            if (forceZeroByHipLock)
            {
                if (female != null)
                    female.setAnimatorParamFloat(paramHash, 0f);
                if (female1 != null && female1.visibleAll && isFemale1)
                    female1.setAnimatorParamFloat(paramHash, 0f);
            }

            // 設定による切断時は従来通り female/female1 を no-op にする。
        ...
        internal bool TryApplyFemaleAnimSpeedCut(ChaControl female, string param, float value)
        {
            bool forceZeroByHipLock = _bodyCtrlLinkEnabled;
            bool cutBySetting = _settings != null && _settings.CutFemaleAnimSpeedEnabled;
            if ((!forceZeroByHipLock && !cutBySetting) || female == null)
                return false;
            ...
            if (forceZeroByHipLock)
                female.setAnimatorParamFloat(param, 0f);
